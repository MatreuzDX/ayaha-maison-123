...
Este documento **não é para ser implementado de uma só vez**. É uma especificação de referência. A implementação faz-se pelo **Plano de Execução por Fases** (secção 32), uma fase por sessão de trabalho.

## 0.2 Divisão de modelos (Opus → Sonnet)

| Tarefa | Modelo | Porquê |
|---|---|---|
| Ler esta spec e desenhar o plano da fase | **Opus** | Decisões de arquitetura, trade-offs, schema |
| Desenhar/alterar o schema Prisma | **Opus** | Erros de modelação são caríssimos de reverter |
| Motor de disponibilidade e deslocamento (secções 11–13) | **Opus** | É o algoritmo central e o mais difícil |
| Motor de comissões e financeiro (secções 20–21) | **Opus** | Dinheiro — não pode ter bugs |
| CRUDs, formulários, tabelas, ecrãs de listagem | **Sonnet** | Repetitivo, padrão bem definido |
| Componentes de UI, estilos, responsividade | **Sonnet** | Padrão já fixado pelo design system |
| Testes unitários e de integração | **Sonnet** | Segue os critérios de aceitação já escritos |
| Seeds e dados de demonstração | **Sonnet** | Mecânico |
| Revisão final de cada fase | **Opus** | Apanhar o que o Sonnet não viu |

**Como trocar:** usar `/model opus` ou `/model sonnet` no Claude Code, ou lançar subagentes com `model: "sonnet"` para o trabalho repetitivo enquanto o Opus mantém o contexto arquitetural.

## 0.3 Regras invioláveis para quem implementa

1. **Nunca inventar campos que não estão no schema.** Se faltar um campo, primeiro atualizar o schema + migração, depois usar.
2. **Toda a lógica de negócio vive em `src/server/services/`**, nunca em componentes React nem em rotas de API. As rotas só validam input, chamam o serviço e formatam a resposta.
3. **Toda a escrita passa por um serviço que grava `AuditLog`.** Sem exceções.
4. **Toda a query multi-tenant filtra por `unitId`.** Mesmo com uma só unidade.
5. **Dinheiro é `Int` em cêntimos.** Nunca `Float`. Nunca `Decimal` em JS.
6. **Datas guardadas em UTC**, convertidas para `Europe/Lisbon` só na apresentação.
7. **Nenhuma dependência nova sem justificação escrita** no PR/commit.
8. **Cada fase termina com `npm run build` + `npm run test` a passar.** Se não passa, a fase não terminou.
...
model Unit {
  ...
  name          String                          // "Benfica — Domicílio"
  slug          String   @unique
  timezone      String   @default("Europe/Lisbon")
  currency      String   @default("EUR")
  // Base a partir da qual as profissionais partem
  baseAddress   String?
  baseLat       Float?
  baseLng       Float?
...
model UserUnit {
  ...
  unitId        String
  role          Role
  // Permissões extra ou removidas face ao papel base
...
// ═══════════════════════════════════════════════
// PROFISSIONAIS (CRM DE FUNCIONÁRIOS)
// ═══════════════════════════════════════════════
...
enum ContractType {
  ...
  RENT_CHAIR        // aluguer de posto (futuro, com espaço físico)
}
...
model Professional {
  ...
  unitId            String
  displayName       String                        // nome comercial, aparece à cliente
  bio               String?
  color             String       @default("#C4A870") // cor na agenda
  contractType      ContractType @default(FREELANCER)
  hireDate          DateTime?
  terminationDate   DateTime?
  // Identificação fiscal (para recibos e comissões)
  taxId             String?                       // NIF
  iban              String?                       // encriptado em repouso
  // Deslocação
...
model ProfessionalSkill {
  ...
  serviceId       String
  // Sobreposições por profissional
  durationMin     Int?     // se null usa a do serviço
  priceCents      Int?     // se null usa a do serviço
  proficiency     Int      @default(3)  // 1-5, usado no auto-atribuir
...
model WorkingHour {
  ...
  endMin          Int
  // Validade do padrão (permite mudar horário a partir de uma data)
...
enum TimeOffType {
  ...
  PERSONAL
  TRAINING
  BLOCKED       // bloqueio pontual sem motivo
  HOLIDAY       // feriado nacional/municipal
}

model TimeOff {
  id              String      @id @default(cuid())
  professionalId  String?     // null = aplica a toda a equipa (ex.: feriado)
  unitId          String
  type            TimeOffType
  startAt         DateTime
  endAt           DateTime
...
enum ClientStatus {
  LEAD              // contactou, nunca marcou
  ACTIVE            // atendida nos últimos 60 dias
  AT_RISK           // 45-90 dias sem voltar
  DORMANT           // >90 dias
  BLOCKED           // no-shows repetidos ou pedido próprio
}
...
model Client {
  ...
  lastName          String?
  email             String?
  phone             String                       // E.164, ex. +351933055502
  phoneWhatsapp     Boolean      @default(true)
  birthDate         DateTime?
  taxId             String?                      // NIF para fatura
  // Morada principal (para deslocação)
  addressLine       String?
  addressExtra      String?                      // andar, porta, código de porta
  postalCode        String?
  ...
  geocodedAt        DateTime?
  travelZoneId      String?
  accessNotes       String?                      // "porta preta, tocar 2x", "sem elevador"
  parkingNotes      String?
  // CRM
  status            ClientStatus @default(LEAD)
  source            AcquisitionSource?
  referredById      String?
  ownerProfessionalId String?                    // profissional preferida
  // Métricas desnormalizadas (recalculadas por job)
  ...
  // Livre
  notes             String?                      // notas internas, não visíveis à cliente
...
model ClientAddress {
  id            String  @id @default(cuid())
  clientId      String
  label         String                 // "Casa", "Trabalho", "Casa da mãe"
  addressLine   String
  addressExtra  String?
  postalCode    String?
  city          String
...
// Dados de saúde — CATEGORIA ESPECIAL RGPD, acesso restrito
model ClientHealthRecord {
  id                String   @id @default(cuid())
  clientId          String   @unique
  allergies         String?
  medications       String?
  eyeConditions     String?              // conjuntivite, blefarite, olho seco
...
model ServiceCategory {
  id        String    @id @default(cuid())
  unitId    String
  name      String                       // "Cílios", "Sobrancelhas"
  slug      String
...
model Service {
  ...
  description       String?
  // Tempos
  durationMin       Int                        // tempo de execução
  setupMin          Int     @default(10)       // montagem no local
  teardownMin       Int     @default(5)        // arrumação
  bufferAfterMin    Int     @default(0)        // descanso extra
  // Preço
  priceCents        Int
  priceOnRequest    Boolean @default(false)
  costCents         Int     @default(0)        // custo de material estimado
  vatBps            Int     @default(2300)     // IVA 23%
  // Regras
  requiresPatchTest Boolean @default(false)
  minAdvanceHours   Int     @default(12)       // antecedência mínima de marcação
  maxAdvanceDays    Int     @default(90)
  requiresDeposit   Boolean @default(false)
  depositCents      Int     @default(0)
  isMaintenance     Boolean @default(false)    // é uma manutenção?
  parentServiceId   String?                    // manutenção de qual serviço
  recommendedGapDays Int?                      // 21 para manutenção de cílios
  // Comissão
  commissionBps     Int?                       // sobrepõe a regra do profissional
  isActive          Boolean @default(true)
  sortOrder         Int     @default(0)
  createdAt         DateTime @default(now())
  updatedAt         DateTime @updatedAt
...
// Consumo padrão de material por serviço (BOM)
model ServiceMaterial {
  id          String   @id @default(cuid())
  serviceId   String
  materialId  String
  quantity    Float                       // ex.: 0.05 frascos de cola
  service     Service  @relation(fields: [serviceId], references: [id])
...
enum AppointmentStatus {
  REQUESTED       // pedido pela cliente, por confirmar
...
model Appointment {
  id                String   @id @default(cuid())
  unitId            String
  code              String   @unique          // AYA-2607-A3F, legível
  clientId          String
  professionalId    String
  ...
  endAt             DateTime
  // Deslocação
  travelToMin       Int      @default(0)      // minutos de viagem até ao local
  travelFromMin     Int      @default(0)
  departAt          DateTime?                 // startAt - travelToMin
  addressId         String?                   // ClientAddress usado
  addressSnapshot   Json?                     // morada congelada no momento
  travelFeeCents    Int      @default(0)
  travelDistanceM   Int?
  // Valores (snapshot no momento da marcação)
  ...
  // Origem
  source            String   @default("MANUAL") // MANUAL | WHATSAPP | WEBSITE | WAITLIST
  createdById       String?
  // Cancelamento
  ...
  clientNotes       String?                   // pedido da cliente
...
model AppointmentItem {
  ...
  serviceId       String
  nameSnapshot    String                  // nome congelado
  durationMin     Int
  unitPriceCents  Int
...
model AppointmentPhoto {
  ...
  storagePath   String
  consentGiven  Boolean  @default(false)  // pode usar em marketing?
  createdAt     DateTime @default(now())
...
model TravelZone {
  id              String  @id @default(cuid())
  unitId          String
  name            String                    // "Lisboa Centro", "Grande Lisboa", "Margem Sul"
  description     String?
  feeCents        Int     @default(0)       // taxa de deslocação
  freeAboveCents  Int?                      // grátis acima deste valor
  minSpendCents   Int     @default(0)       // valor mínimo para aceitar
  estimatedMin    Int                       // tempo típico a partir da base
  maxRadiusKm     Float?
  postalPrefixes  String[] @default([])     // ["1500","1600","1700"]
  color           String  @default("#C4A870")
  isActive        Boolean @default(true)
  sortOrder       Int     @default(0)
  @@unique([unitId, name])
...
// Cache de estimativas da API de mapas
model TravelEstimate {
  id            String   @id @default(cuid())
  originHash    String                     // geohash precisão 7
  destHash      String
  mode          String                     // DRIVING | TRANSIT
  timeBand      String                     // OFFPEAK | MORNING_PEAK | EVENING_PEAK
  durationMin   Int
  distanceM     Int
  provider      String
...
model WaitlistEntry {
  ...
  clientId          String
  serviceIds        String[]
  professionalId    String?                  // null = qualquer
  // Janelas desejadas
  earliestDate      DateTime
  latestDate        DateTime
  weekdays          Int[]    @default([])    // dias aceitáveis
  preferredStartMin Int?                     // ex.: 540 = a partir das 09:00
  preferredEndMin   Int?
...
model TimelineEvent {
  ...
  unitId      String
  clientId    String?
  entityType  String                       // CLIENT | APPOINTMENT | INVOICE | ...
  entityId    String
  type        String                       // APPOINTMENT_BOOKED, MESSAGE_SENT, NOTE_ADDED...
  title       String
  body        String?
  icon        String?
  actorId     String?
...
enum MessageChannel { WHATSAPP EMAIL SMS INTERNAL }
...
model MessageTemplate {
  id            String   @id @default(cuid())
  unitId        String
  key           String                     // APPOINTMENT_CONFIRMED, REMINDER_24H...
  channel       MessageChannel
  name          String
  subject       String?                    // e-mail
  body          String                     // com {{variáveis}}
  providerRef   String?                    // nome do template aprovado na Meta
...
model Message {
  ...
  subject       String?
  body          String
  providerId    String?                    // id da mensagem no fornecedor
  errorMessage  String?
...
// AYAHA Club — cartão de carimbos (NÃO é sistema de pontos)
model LoyaltyProgram {
  id                String   @id @default(cuid())
  unitId            String   @unique
  name              String   @default("AYAHA Club")
  isActive          Boolean  @default(true)
  // Regra real: 5 atendimentos concluídos = 1 recompensa à escolha
  stampsRequired    Int      @default(5)
  // Após completar, o cartão recomeça do zero
  autoRestart       Boolean  @default(true)
  // Quantos dias a recompensa ganha fica válida
...
// Um cartão por ciclo. Cliente com 12 atendimentos tem 2 cartões completos + 1 em curso.
model LoyaltyCard {
  id            String   @id @default(cuid())
  programId     String
  clientId      String
  cycleNumber   Int      @default(1)     // 1º, 2º, 3º cartão da cliente
  stampsCount   Int      @default(0)
  stampsRequired Int                     // congelado à abertura (regra pode mudar)
  completedAt   DateTime?
  isActive      Boolean  @default(true)  // só um cartão ativo por cliente
  createdAt     DateTime @default(now())
  updatedAt     DateTime @updatedAt
...
model LoyaltyStamp {
  id            String   @id @default(cuid())
  cardId        String
  appointmentId String?  @unique          // 1 atendimento = no máximo 1 carimbo
  reason        String   @default("APPOINTMENT") // APPOINTMENT | MANUAL | COMPENSATION
  note          String?
  createdById   String?
  createdAt     DateTime @default(now())
  revokedAt     DateTime?                 // estorno de atendimento revoga o carimbo
...
enum RewardKind {
  DISCOUNT_FIXED      // €15 de desconto no próximo atendimento
  SERVICE_UPGRADE     // upgrade de técnica gratuito
  GIFT_CARD           // gift card de €15 para oferecer
}
...
model Reward {
  ...
  description   String?
  kind          RewardKind
  valueCents    Int        @default(0)   // 1500 para as de €15
  sortOrder     Int        @default(0)
  isActive      Boolean    @default(true)
...
enum RedemptionStatus { AVAILABLE USED EXPIRED CANCELLED }

model RewardRedemption {
  id            String   @id @default(cuid())
  unitId        String
  cardId        String   @unique          // um cartão completo gera uma recompensa
  rewardId      String
  clientId      String
  code          String   @unique          // AYA-J8AU, apresentado no atendimento
  status        RedemptionStatus @default(AVAILABLE)
  chosenAt      DateTime @default(now())
  expiresAt     DateTime?
  usedAt        DateTime?
  usedAppointmentId String?
  // Se for GIFT_CARD, o gift card gerado
...
model GiftCard {
  id              String   @id @default(cuid())
  unitId          String
  code            String   @unique          // AYA-GIFT-7K2M
  initialCents    Int
  balanceCents    Int
...
enum CouponKind { PERCENT FIXED FREE_SERVICE }
...
model Coupon {
  ...
  name            String
  kind            CouponKind
  valueBps        Int?                     // se PERCENT
  valueCents      Int?                     // se FIXED
  serviceId       String?                  // se FREE_SERVICE
...
model Material {
  ...
  supplierId        String?
  sku               String?
  name              String                    // "Cola Premium 5ml"
  brand             String?
  category          String?                   // COLA | FIOS | PINCAS | DESCARTAVEL | LIMPEZA
  unit              String   @default("un")   // un, ml, g, par
  // Stock
  quantityOnHand    Float    @default(0)
  minQuantity       Float    @default(0)      // alerta abaixo disto
  reorderQuantity   Float    @default(0)
  // Custo
  costCents         Int      @default(0)      // custo unitário médio
...
enum StockMovementKind { PURCHASE CONSUMPTION ADJUSTMENT LOSS RETURN TRANSFER }
...
enum InvoiceStatus { DRAFT ISSUED PAID PARTIAL VOID REFUNDED }
enum PaymentMethod { CASH CARD MBWAY TRANSFER GIFT_CARD LOYALTY OTHER }

model Invoice {
  id                String   @id @default(cuid())
  unitId            String
  number            String                    // sequencial por ano: 2026/0001
  clientId          String
  ...
  // Dados fiscais congelados
  clientNameSnapshot String
  clientTaxIdSnapshot String?
  externalRef       String?                   // nº no software certificado da AT
  notes             String?
  createdAt         DateTime @default(now())
  updatedAt         DateTime @updatedAt
  client            Client   @relation(fields: [clientId], references: [id])
...
model Payment {
  ...
  amountCents   Int
  reference     String?                     // ref MBWay, últimos 4 dígitos
  giftCardId    String?
  receivedAt    DateTime @default(now())
  receivedById  String?
...
// Caixa: abertura/fecho do dia por profissional
model CashSession {
  id                String   @id @default(cuid())
  unitId            String
  professionalId    String?
...
model ExpenseCategory {
  id        String    @id @default(cuid())
  unitId    String
  name      String                          // Material, Transporte, Marketing, Formação
...
enum CommissionBasis { SERVICE_REVENUE NET_REVENUE FIXED_PER_SERVICE }

model CommissionRule {
  id              String   @id @default(cuid())
  unitId          String
  professionalId  String?                   // null = regra por defeito da unidade
  serviceId       String?                   // null = todos os serviços
  categoryId      String?
  basis           CommissionBasis @default(SERVICE_REVENUE)
  rateBps         Int      @default(4000)   // 40%
  fixedCents      Int      @default(0)
  // Escalões por faturação mensal
  tierFromCents   Int      @default(0)
  tierToCents     Int?
  // Deduções
  deductMaterials Boolean  @default(false)
  deductTravel    Boolean  @default(false)
  priority        Int      @default(0)      // maior ganha
  validFrom       DateTime @default(now())
  validUntil      DateTime?
  isActive        Boolean  @default(true)
  professional    Professional? @relation(fields: [professionalId], references: [id])
...
model Commission {
  ...
  invoiceId       String?
  ruleId          String?
  baseCents       Int                       // valor sobre o qual incidiu
  rateBps         Int
  amountCents     Int
...
model Signature {
  ...
  documentId      String
  signerName      String
  signerRole      String                    // CLIENT | PROFESSIONAL | GUARDIAN
  clientId        String?
  imagePath       String                    // PNG do canvas
  signedAt        DateTime @default(now())
  ip              String?
  userAgent       String?
  documentHash    String                    // SHA-256 do documento assinado
  document        Document @relation(fields: [documentId], references: [id])
  @@index([documentId])
}

enum ConsentPurpose {
  SERVICE_DELIVERY      // execução do serviço (base contratual)
  HEALTH_DATA           // dados de saúde (consentimento explícito)
  MARKETING_EMAIL
  MARKETING_WHATSAPP
  PHOTOS_INTERNAL
  PHOTOS_PUBLIC         // usar fotos em redes sociais
  DATA_RETENTION
}
...
model Consent {
  ...
  granted       Boolean
  version       String                      // versão do texto aceite
  textSnapshot  String                      // texto exato mostrado
  method        String                      // WEB_FORM | SIGNATURE | VERBAL_RECORDED
  documentId    String?
  ip            String?
  grantedAt     DateTime?
  revokedAt     DateTime?
...
// Pedidos de titular de dados (RGPD art. 15-22)
model DataRequest {
  id            String   @id @default(cuid())
  unitId        String
  clientId      String?
  requesterEmail String
  kind          String                      // ACCESS | RECTIFY | ERASE | PORTABILITY | OBJECT
  status        String   @default("RECEIVED") // RECEIVED | VERIFYING | PROCESSING | DONE | REJECTED
  notes         String?
  resultPath    String?
  dueAt         DateTime                    // recebido + 30 dias
  completedAt   DateTime?
  handledById   String?
  createdAt     DateTime @default(now())
...
model JobQueue {
  id            String   @id @default(cuid())
  kind          String                      // SEND_REMINDER, RECALC_METRICS, EXPIRE_POINTS...
  payload       Json
  runAt         DateTime
...
model Notification {
  ...
  unitId      String
  userId      String
  kind        String                        // LOW_STOCK, NEW_BOOKING, NO_SHOW...
  title       String
  body        String?
  link        String?
...
## ANEXO B — Prompt de arranque para o Claude Code

Colar isto no início de cada sessão de implementação:

```
Vamos implementar o AYAHA CRM.

Especificação: docs/ESPECIFICACAO.md — LÊ-A antes de escrever código.
Fase atual: <N> — <nome>
Módulos desta fase: <lista>

Regras invioláveis (secção 0.3 da spec):
1. Lógica de negócio só em src/server/services/
2. Toda a escrita grava AuditLog na mesma transação
3. Toda a query filtra por unitId
4. Dinheiro em Int de cêntimos, nunca Float
5. Datas em UTC, apresentadas em Europe/Lisbon
6. Textos da UI em pt-PT

Antes de codar:
- Se o módulo tem 🔎, faz a pesquisa da secção 3 e grava em docs/pesquisa/
- Apresenta-me o plano da fase e espera aprovação

No fim:
- Corre lint, typecheck, test e build
- Confirma cada critério de aceitação da spec, um a um
- Atualiza CLAUDE.md
```
